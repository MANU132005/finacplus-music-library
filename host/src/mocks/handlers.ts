...
export const handlers = [
  ...
  http.get('https://itunes.apple.com/search', async ({ request }) => {
    try {
      // Perform a real fetch to iTunes (bypassing MSW interceptor)
      ...
      // Merge added songs and filter out deleted ones
      const mergedSongs = [...added, ...itunesSongs].filter(
        (song) => !deleted.includes(song.id)
      );

      // Return unified results matching the structure that our service layer expects
      return HttpResponse.json({ results: mergedSongs });
    } catch (error) {
      // In case of offline/network errors, fallback to returning only custom added songs
  ...
  http.post('/songs', async ({ request }) => {
    ...
    const newSong: Song = {
      ...
      coverUrl: 'https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?w=100&h=100&fit=crop&q=80', // Premium visual placeholder
    };
  ...
  http.delete('/songs/:id', ({ params }) => {
    ...
    if (index > -1) {
      // If it's a custom song, delete it entirely from localStorage added songs
      added.splice(index, 1);
      localStorage.setItem(ADDED_SONGS_KEY, JSON.stringify(added));
    } else {
      // If it's an iTunes song, add it to the deleted list so it gets filtered out
      const deleted = getDeletedSongs();
